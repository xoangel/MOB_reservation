...
const { activeStage, stagesDataStored, loading } = storeToRefs(appStore);
...
const confirm = () => {
    stagesDataStored.value[activeStage.value].tripId = seletedTrip.value.documentId;
    appStore.nextStage();
}
        ...
        <div class="flex flex-wrap w-full gap-2">
            <TransitionGroup name="staggerY">
                <div v-for="trip in trips" :key="trip.documentId"
                    class="trip relative w-full h-60 rounded-xl border border-solid border-neutral-100 overflow-hidden transition-all duration-300 ease-out"
        ...
        <Transition name="fade" mode="out-in">
            <div v-if="showInfo && !showMap"
                ...
                <Button :label="`Выбрать ${seletedTrip.title.toLowerCase()}`" severity="info" icon="pi pi-check" raised
                    fluid variant="outlined" class="mt-2.5" @click="confirm"></Button>
            </div>
            <div v-else-if="showMap" class="w-full relative">
                <Button label="Назад" severity="info" icon="pi pi-arrow-left" raised class="absolute left-3 top-14" @click="showMap = false"></Button>
                <iframe class="rounded-xl border border-solid border-neutral-100" src="https://yandex.ru/map-widget/v1/?um=constructor%3A355e2c787c3cbf345abb1f58a9b66a22a921d4379b1d230c3aac517b43570d63&amp;source=constructor" width="100%" height="556" frameborder="0"></iframe>
            </div>
        </Transition>
    </div>
</template>
